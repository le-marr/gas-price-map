...
const REMOTE_DATA_URL = 'https://regieessencequebec.ca/stations.geojson.gz';
const CACHE_KEY = 'stations-data-remote-json';
...
class JsonDataService {
    public async getFuelData(): Promise<FuelData> {
        ...
        console.log('Fetching remote GeoJSON data from:', REMOTE_DATA_URL);
        const response = await fetch(REMOTE_DATA_URL);
        if (!response.ok) {
            throw new Error(`Failed to fetch remote GeoJSON: ${response.statusText}`);
        }
